...
if (stop - start) > 10 ** -8 and step > 10 ** -8:
    ...
    if 4 <= points <= 8:
        ...
        pr_d = (start - step) ** 5 - 7.9 * (start - step) ** 4 + 24.46 * (start - step) ** 3 \
               - 37.074 * (start - step) ** 2 + 27.512 * (start - step) - 8.0042
        max_diff = min_diff = None
        for i in range(iters):
            y = round(start + step * i, 13)
            d = y ** 5 - 7.9 * y ** 4 + 24.46 * y ** 3 \
                - 37.074 * y ** 2 + 27.512 * y - 8.0042
            ...
            pr_d = d
            if max_diff is None:
                max_diff = min_diff = d_diff
            else:
                max_diff = max(max_diff, d_diff)
                min_diff = min(min_diff, d_diff)

        print("\n\n  f1'", ' ' * 5, end='|')
        pad = round((95 - points * 11) / (points - 1))
        for i in range(points):
            if i == 0:
                print('{:<11.5g}'.format(i * ((max_diff - min_diff)
                                              / (points - 1)) + min_diff),
                      end=' ' * (pad if points != 6 else pad - 1))
            elif i == points - 1:
                print('{:>11.5g}'
                      .format(i * ((max_diff - min_diff)
                                   / (points - 1)) + min_diff), end='|\n')
            else:
                print('{:^11.5g}'
                      .format(i * ((max_diff - min_diff)
                                   / (points - 1)) + min_diff), end=' ' * pad)
        print('-' * 11, '|', '-' * 95, '|', sep='')
        ...
        for i in range(iters):
            ...
            d_diff = (d - pr_d) / step
            pr_d = d
            index = round((d_diff - min_diff) / (max_diff - min_diff) * 94)
            if min_diff - 10 ** -6 <= 0 <= max_diff + 10 ** -6:
                ind_zero = round((-min_diff) / (max_diff - min_diff) * 94)
                if ind_zero < index:
                    print('{:<11.5g}|'.format(y), ind_zero * ' ', '|',
                          (index - ind_zero - 1) * ' ',
                          '*', (94 - index) * ' ', '|', sep='')
